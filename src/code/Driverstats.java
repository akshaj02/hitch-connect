...
public class Driverstats {
    ...
    public static boolean find_driver(String full_Name){
        ...
        for(int i = 0; i < fileFunctions.numberOfLines("info.txt"); i++){
            try {
                String line = Files.readAllLines(Paths.get("otherFiles/info.txt")).get(lineNumber);
            ...
            public void actionPerformed(ActionEvent e) {
                if (e.getSource() == button1) {
                    if(find_driver(textfield.getText())){
                        JOptionPane.showMessageDialog(null, "Driver found: " + userAndPass[0] + " drives a " + userAndPass[1]);
                    }
                    else{
